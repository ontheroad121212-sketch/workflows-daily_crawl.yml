...
def collect_hotel_data(driver, hotel_name, hotel_id, target_date, is_precision_mode):
    ...
    try:
        ...
        # [핵심 수술] 네이버가 데이터를 숨겨놨을지 모르니 더 깊게 스크롤하고 기다립니다
        driver.execute_script("window.scrollTo(0, 1500);")
        time.sleep(5)
        
        # 모든 가능한 객실 상자 후보군을 다 잡습니다
        items = driver.find_elements(By.CSS_SELECTOR, "li[class*='item'], div[class*='item'], li[class*='Rate']")
        
        print(f"      🔎 탐색 시작 (후보군 {len(items)}개 발견)...", flush=True)

        rows = []
        now = datetime.now().strftime("%Y-%m-%d %H:%M")
        
        for i, item in enumerate(items):
            # [추가] 6개 상자 하나하나에 뭐가 들어있는지 강제로 출력해봅니다
            raw_content = item.text.replace("\n", " | ").strip()
            if not raw_content:
                # 텍스트가 안 잡히면 HTML 내부라도 긁어봅니다
                raw_content = item.get_attribute('innerText').replace("\n", " | ").strip()
            
            print(f"      📦 {i+1}번 상자 내용: {raw_content[:100]}...", flush=True)

            if "원" not in raw_content: continue

            parts = [p.strip() for p in raw_content.split("|")]
            room_name = parts[0]

            # 엠버 10종 필터 (일단 로그를 보기 위해 잠시 통과시키거나 느슨하게 잡음)
            amber_types = ["그린밸리", "포레스트", "힐파인", "힐엠버", "힐루나", "풀빌라"]
            
            clean_rn = room_name.replace(" ", "")
            if hotel_name == "엠버퓨어힐":
                if not any(target in clean_rn for target in amber_types):
                    continue

            # 가격 추출
            prices = [int(re.sub(r'[^0-9]', '', p)) for p in parts if "원" in p and re.sub(r'[^0-9]', '', p)]
            if prices and max(prices) > 100000:
                real_p = max(prices)
                rows.append([now, hotel_name, room_name, "네이버수집", real_p, target_date])
                print(f"    🔎 [성공] {room_name}: {real_p:,}원", flush=True)
        
        return rows
    except Exception as e:
        print(f"❌ {hotel_name} 오류: {e}", flush=True); return []
